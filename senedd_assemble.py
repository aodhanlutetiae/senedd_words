...
# 4. Assemble a dictionary of {YEAR:[{word:freq, word:freq}}]}
...
def year_to_list(yr, df):
    ...
    # get a series from the column containing the text, and make into a list
    df_series = fil.p
    ...
    c = Counter(non)
    year_freq = [(i, c[i] / len(non) * 100.0) for i in c] 
    
    return year_freq
    
# make a YEAR LIST from the df, & order it so we have keys for each year
years = list(mega_df.year.unique())
years_int = [x for x in years]
years_int.sort()
